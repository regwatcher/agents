...
def reset_sandbox() -> None:
    ...
    # Keep this a standalone uv project. If it joins the parent workspace,
    # `uv add` recreates the crew's .venv (and can switch Python versions)
    # while the crew is already running. Drop VIRTUAL_ENV so uv does not
    # target the parent environment.
    uv_env = os.environ.copy()
    uv_env.pop("VIRTUAL_ENV", None)
    subprocess.run(
        ["uv", "init", "--bare", "--python", "3.13", "--no-workspace"],
        cwd=SANDBOX_DIR,
        check=True,
        env=uv_env,
    )
